...
export const PLUGINS_ROOT = path.join(ROOT_PATH, 'libs/');
export const PLUGIN_PATHS = fs.readdirSync(PLUGINS_ROOT).map(d => path.join(PLUGINS_ROOT, d, 'package.json'));

export const DEST_ROOT = path.join(ROOT_PATH, 'dist/libs/');
...
export function preparePackage(name) {
    ...
    projectTasks.push({
        ...
        task: async () => {
            try {
                const data = await execa.shell(`npm run build:${name}`, { cwd: ROOT_PATH });
            } catch (error) {
...
export function changeJson(name) {
    ...
    const packageJsonContents = mergePackageJsonContent(packageJson);
